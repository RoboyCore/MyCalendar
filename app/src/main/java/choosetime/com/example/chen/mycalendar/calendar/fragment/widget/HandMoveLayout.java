...
import choosetime.com.example.chen.mycalendar.calendar.adpter.CalendarBaseAdapter;
...
public class HandMoveLayout extends LinearLayout {
    ...
    private boolean isWeekSelected = true;//是否选中周历
...
//                }
     ...
     * @param deltaY
     */
    public void smoothExpand(int deltaY) {
...
//                        mHeader.setLayoutParams(params);
     ...
     * 设置展开月历
     */
    private void setExpandMonth() {
        if (!isWeekSelected) {
            moveToScroll(-600);//TODO：转换成DP
            if (monthViewpager.getVisibility() != VISIBLE) {
                if (true) {
                    monthViewpager.setVisibility(VISIBLE);
                    weekviewpager.setVisibility(GONE);
